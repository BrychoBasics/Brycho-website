...
function downloadAsJSON(data) {
    ...
    alert('JSON downloaded! Add this to inspiration-images.json and upload your images to /images/ folder.');
    resetForm();
}
...
function renderQueue(submissions) {
    ...
    if (submissions.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: var(--color-text-muted);">No pending submissions</p>';
        return;
    }
...
function renderContentGrid(images) {
    ...
    if (images.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: var(--color-text-muted);">No images yet</p>';
        return;
    }
...
window.rejectSubmission = async function(id) {
    if (!confirm('Reject this submission? This will delete it permanently.')) return;
